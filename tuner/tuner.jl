...
@everywhere begin
    ...
    function generate_cacti_config(c::Configuration, path::String; is_final::Bool = false)
        filename  = ""

        if is_final
            filename = "$path/final_tuned.cfg"
        else
...
println("Generating final config at $results_path")
generate_cacti_config(result.minimum, results_path, is_final = true)
