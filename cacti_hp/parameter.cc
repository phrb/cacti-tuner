  ...
  is_main_mem(is_main_mem_), cell(), is_valid(false)
{
  ...
  else
  {
    ...
    else
    {
     ...
     // cout << capacity_per_die << " " << g_ip->nbanks << " " << g_ip->block_sz << " " 
  ...
  if ((!fully_assoc)&&(num_r_subarray < MINSUBARRAYROWS)) return;//{ cout << capacity_per_die<< " " << g_ip->nbanks << " "<< num_r_subarray << " here " << endl; return;}
  
  if (num_r_subarray == 0) return;
  //cout << "here " << g_ip->cache_sz << "/" << NUMBER_STACKED_DIE_LAYERS << " " <<  num_r_subarray << endl;
